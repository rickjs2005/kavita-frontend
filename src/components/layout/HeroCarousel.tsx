...
function SlideContent({ slide, isFirst }: { slide: HeroSlide; isFirst: boolean }) {
  ...
  return (
    <>
      {/* Badge */}
      {slide.badge_text ? (
        <div className={`mb-5 sm:mb-6 inline-flex items-center gap-2 rounded-lg border bg-black/30 px-3.5 py-1.5 backdrop-blur-md text-[13px] font-semibold tracking-widest uppercase ${typeAccent[slide.slide_type] || "border-primary/40 text-primary"}`}>
          {slide.badge_text}
        </div>
      ) : null}

      {/* Title */}
      <Heading className="text-[clamp(2rem,6vw,4.5rem)] font-black leading-[0.92] tracking-tight text-white [text-shadow:0_2px_20px_rgba(0,0,0,0.5)]">
        {titleText}
      </Heading>

      {/* Divider accent */}
      <div className="mt-5 h-1 w-16 rounded-full bg-primary sm:mt-6 sm:w-20" />

      {/* Subtitle */}
      {subtitleText ? (
        <p className="mt-4 max-w-xl text-[clamp(0.95rem,1.5vw,1.25rem)] leading-relaxed text-white/75 sm:mt-5">
          {subtitleText}
        </p>
      ) : null}
        ...
        {secondaryHref && slide.button_secondary_label ? (
          <Link
            ...
            className="
              ...
              rounded-xl border border-white/20 bg-white/[0.07] px-7 py-3.5
              text-[15px] font-semibold text-white/85
              backdrop-blur-sm
...
export default function HeroCarousel({ slides }: Props) {
  ...
  return (
    <section
      className="relative w-full overflow-hidden min-h-[60vh] sm:min-h-[80vh] lg:min-h-[92vh] flex flex-col"
    ...
    >
      ...
      {/* ── Overlay system ── */}
      {/* Base darkening — stronger at bottom for text, lighter at top to show media */}
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 via-55% to-black/20" />
      ...
      {/* Subtle brand-color ambient at top */}
      <div className="absolute inset-0 [background:radial-gradient(ellipse_70%_40%_at_50%_0%,rgba(53,146,147,0.08),transparent_70%)]" />

      {/* ── Main content area ── */}
      <div className="relative z-10 flex flex-1 flex-col justify-center px-5 sm:px-8 lg:px-14">
      ...
      {total > 1 ? (
        <div className="relative z-10 px-5 pb-6 sm:px-8 sm:pb-8 lg:px-14">
          <div className="mx-auto flex w-full max-w-7xl items-center gap-5">
            {/* Progress indicators */}
            <div className="flex flex-1 items-center gap-1.5" key={progressKey}>
              {effectiveSlides.map((_, i) => (
                <button
                  key={i}
                  type="button"
                  onClick={() => goTo(i)}
                  className="group relative h-1 flex-1 max-w-[120px] overflow-hidden rounded-full bg-white/15"
            ...
            <div className="flex items-center gap-2.5">
              ...
              <button
                ...
                className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/15 bg-white/[0.06] text-white/60 hover:bg-white/15 hover:text-white backdrop-blur-sm transition-all sm:h-10 sm:w-10"
