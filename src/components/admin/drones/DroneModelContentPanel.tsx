...
export default function DroneModelContentPanel() {
  ...
  async function loadModels() {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/admin/drones/models?includeInactive=1`, {
        credentials: "include",
      });
      ...
      if (!res.ok) throw new Error(data?.message || "Erro ao listar modelos.");
      ...
      // auto-select primeiro ativo se nada selecionado
      if (!selectedKey) {
  ...
  async function savePick(modelKey: string, target: "HERO" | "CARD", mediaId: number) {
    const token = localStorage.getItem("adminToken"); // ou de onde vc pega
  ...
  return (
    <div className="space-y-5">
            ...
            {loadingModel ? (
              ...
              <div className="mt-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div className="text-sm text-white/70">
                  Editando:{" "}
                  <b className="text-white">{modelLabel || selectedKey.toUpperCase()}</b>
                </div>
                {lastUpdate ? (
      ...
      {/* Conteúdo por modelo */}
      {selectedKey ? (
        // ✅ AQUI está a correção: empilhado, 1 coluna sempre (responsivo)
        <div className="flex flex-col gap-5">
          ...
          {/* ✅ Galeria por modelo (reativada) */}
          <SectionShell title="Galeria" subtitle={`Por modelo: ${selectedKey.toUpperCase()}`}>
            <GalleryForm
              modelKey={selectedKey}
              currentCardMediaId={selected?.current_card_media_id ?? null}
              currentHeroMediaId={selected?.current_hero_media_id ?? null}
              onPickForCard={async (item) => {
                await savePick(selectedKey, "CARD", item.id);
                await loadModels(); // ✅ recarrega pra refletir seleção no UI
              }}
