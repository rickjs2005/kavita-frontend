...
export default function HeroAtendimento({ whatsappUrl, config }: Props) {
  const badge = config?.hero_badge ?? "Central de Ajuda Kavita";
  const title = config?.hero_title ?? "Precisa de ajuda?";
  const highlight = config?.hero_highlight ?? "Estamos com voce.";
  const description =
    config?.hero_description ??
    "Tire duvidas sobre pedidos, entregas, trocas ou pagamentos. Nossa equipe responde rapido para voce resolver tudo sem complicacao.";
  const ctaPrimary = config?.hero_cta_primary ?? "Falar pelo WhatsApp";
  const ctaSecondary = config?.hero_cta_secondary ?? "Enviar mensagem";
  const status = config?.hero_status ?? "Atendimento ativo agora";
  const schedule = config?.hero_schedule ?? "Seg a sex, 8h as 18h";
  const sla = config?.hero_sla ?? "Resposta em ate 24h";
      ...
      <div className="relative mx-auto max-w-5xl px-4 pb-14 pt-14 sm:pb-20 sm:pt-20">
        <div className="text-center">
          ...
          <h1 className="text-3xl font-extrabold leading-[1.15] tracking-tight text-white sm:text-4xl lg:text-[3.25rem]">
            {title}
            <br />
            <span className="text-teal-light">{highlight}</span>
          </h1>
