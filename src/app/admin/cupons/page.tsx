...
export default function CuponsPage() {
    ...
    async function carregarCupons() {
        ...
        if (!headers) {
            setError("Faça login no painel admin.");
            setLoading(false);
            return;
        }
    ...
    async function handleRemoverCupom(id: number) {
        ...
        try {
            ...
            console.error(err);
            toast.error(err.message || "Erro ao remover o cupom.");
            setError(err.message || "Erro ao remover o cupom.");
        }
    }
                            ...
                            <div className="grid gap-3 sm:grid-cols-2">
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        min={0}
                                        value={form.minimo}
                                        onChange={(e) => handleChange("minimo", Number(e.target.value))}
                                        className="w-full rounded-lg border border-white/10 bg-[#020617] px-3 py-2 text-sm text-white outline-none focus:border-[#38bdf8]"
                                        placeholder="0"
                                    />
                                    <p className="text-[11px] text-gray-400">
                                                ...
                                                {(() => {
                                                    ...
                                                    return (
                                                        <>
                                                            ...
                                                            {minimo > 0 && <> · Mínimo R$ {minimo.toFixed(2)}</>}
                                                        </>
                                                    );
                                                })()}
                                            </div>
